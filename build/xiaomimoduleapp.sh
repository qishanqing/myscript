...
init_project_env(){
    ...
    appname=XIAOMI-MODULE
    sourcename=xiaomi-module
    ...
    CLONE_DEPTH="--depth=1"
    CONFIG_REMOTE="git clone ssh://git@192.168.50.191:222/AroundI18RProject/i18rconfig $CONFIG_DIR -b xiaomi_module  $CLONE_DEPTH"
...
function App_project_fetch(){
	...
	mkdir -p $WORK_DIR
	pushd $BUILD_DIR
	git clone ssh://git@192.168.50.191:222/xiaomi/integration/package.git $sourcename -b ${INTEGRATION_BRANCH} && (
	    pushd $sourcename
