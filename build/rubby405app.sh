...
init_project_env(){
    ...
    PLATFORM=`uname -m`
    CLONE_DEPTH="--depth=1"
    CONFIG_REMOTE="git clone ssh://git@192.168.50.191:222/AroundI18RProject/i18rconfig $CONFIG_DIR -b rubby405-dev $CLONE_DEPTH"
...
function App_project_fetch(){
	...
	mkdir -p $WORK_DIR
	pushd $BUILD_DIR
	git clone ssh://git@192.168.50.191:222/rubby405/integration/rubby_demo.git $sourcename -b ${SDK_BRANCH:-master} && (
	    pushd $sourcename
