...
function generate_commits(){
	...
	cp -ar $sync_files .
	deploy_project $sync_files
    elif ! [ -z $Modules_List ] || [[ $JOB_NAME =~ i18r_g.*-sdk ]];then
...
function public_project_update(){
    ...
    if [[ $JOB_NAME =~ "$JENKINS_JOB_G" ]] || [[ $JOB_NAME =~ "$JENKINS_JOB_E" ]];then
