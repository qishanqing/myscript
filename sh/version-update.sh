...
#    for file in `cat $CONFIG_DIR/encryption.list`;do find $WORK_DIR -name $file | xargs -i readlink -f {} | while read so; do echo $so;done;done
    ...
    find  $WORK_DIR -type l -name libPSL.so | xargs -i readlink -f {} | while read so;do $ENCRYPTION_DEAO_TOOL $so -c $ENCRYPTION_TOOL_CONFIG -o $so.bak;rm -f $so;mv $so.bak $so;done
