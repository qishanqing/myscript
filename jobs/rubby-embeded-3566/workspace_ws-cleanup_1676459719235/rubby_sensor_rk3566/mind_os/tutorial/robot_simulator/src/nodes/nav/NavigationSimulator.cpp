...
public:
    NavigationSimulator()
    {
        ...
        getPoseThread = new mind_os::util::LoopThread(25, [this](){
            LOG(INFO) << "wait for get pose in sub thread.";
            GetPoseSrv srv;
            srv.request.timestamp = mind_os::util::now();
            if (scGetPose.call(srv))
            {
                LOG(INFO) << "success to get pose in sub thread, time:" << srv.response.pose.header.stamp 
                    << ", latency: " << (srv.response.pose.header.stamp - srv.request.timestamp) << " us";
            }
            else
            {
                LOG(INFO) << "failed to get pose in sub thread.";
            }
            LOG(INFO) << "finished waiting for get pose in sub thread.";
