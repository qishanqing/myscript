...
namespace mind_os
{
    ...
    inline void serialize(const nav_srvs::CleanRegion::Request& data, BytesBuffer& buffer)
    {
        ...
        serialize(data.areaForbiden, buffer);
        serialize(data.wallForbiden, buffer);
        serialize(data.regionLimitFromApp, buffer);
        serialize(data.regionLimitFromPerception, buffer);
    }
