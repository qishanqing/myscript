...
void serialize(const tutorial::slam::SlamResult& data, BytesBuffer& buffer) {
    ...
    serializeVector(data.s_rotation, buffer);
    serializeVector(data.s_position, buffer);
...
void deserialize(BytesBuffer& buffer, tutorial::slam::SlamResult& data) {
    ...
    deserializeVector(buffer, data.s_rotation);
    deserializeVector(buffer, data.s_position);
