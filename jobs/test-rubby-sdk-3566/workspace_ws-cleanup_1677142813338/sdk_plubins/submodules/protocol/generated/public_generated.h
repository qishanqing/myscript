...
enum CleanState {
  ...
  CleanState_Stop = 3,
  CleanState_NewScene = 4,
  CleanState_MIN = CleanState_Start,
  CleanState_MAX = CleanState_NewScene
};
...
inline const char *EnumNameCleanState(CleanState e) {
  if (flatbuffers::IsOutRange(e, CleanState_Start, CleanState_NewScene)) return "";
  const size_t index = static_cast<size_t>(e);
...
enum CleanMode {
  ...
  CleanMode_MAX = CleanMode_Stop
};

inline const CleanMode (&EnumValuesCleanMode())[3] {
...
inline const char *EnumNameCleanMode(CleanMode e) {
  if (flatbuffers::IsOutRange(e, CleanMode_Quiet, CleanMode_Stop)) return "";
  const size_t index = static_cast<size_t>(e);
...
struct Outline FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyField<int32_t>(verifier, VT_CLASS_ID) &&
...
struct OutlineBuilder {
  ...
  void add_alias(flatbuffers::Offset<flatbuffers::String> alias) {
    fbb_.AddOffset(Outline::VT_ALIAS, alias);
  }
  void add_object_id(int32_t object_id) {
    fbb_.AddElement<int32_t>(Outline::VT_OBJECT_ID, object_id, 0);
  }
  void add_class_id(int32_t class_id) {
    fbb_.AddElement<int32_t>(Outline::VT_CLASS_ID, class_id, 0);
  }
  void add_stype(uint16_t stype) {
    fbb_.AddElement<uint16_t>(Outline::VT_STYPE, stype, 0);
  }
