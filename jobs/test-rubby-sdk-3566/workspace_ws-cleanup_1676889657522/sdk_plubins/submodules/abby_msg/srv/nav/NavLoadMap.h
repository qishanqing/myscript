...
namespace mind_os
{
    ...
    inline void serialize(const nav_srvs::NavLoadMap::Request& data, BytesBuffer& buffer)
    {
        ...
        serialize(strlen(data.loadMapPath), buffer);
        serialize(data.loadMapPath, strlen(data.loadMapPath) + 1, buffer);
    ...
    inline void deserialize(BytesBuffer& buffer, nav_srvs::NavLoadMap::Request& data)
    {
        ...
        data.loadMapPath = new char[size];
        deserialize(buffer, data.loadMapPath, size);
