...
inline hsm_bool StateMachine::ProcessStateTransitionsOnce()
{
	...
	{
		// LOGHSM(INFO) << "前  depth: " << depth << "  mStateStack.size():" << mStateStack.size();
		State* currState = GetStateAtDepth(depth);
		// LOGHSM(INFO) << "currState: " << currState;
		const Transition& transition = currState->GetTransition();
		// LOGHSM(INFO) << "中  depth: " << depth << "  mStateStack.size():" << mStateStack.size();
		...
		// LOGHSM(INFO) << "后  depth: " << depth << "  mStateStack.size():" << mStateStack.size();
	} // end for each depth
