...
void DistanceSegmentation::segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map, double map_resolution_from_subscription, 
double room_area_factor_lower_limit, double room_area_factor_upper_limit,std::vector < std::vector<cv::Point> > &saved_contours)
{
	...
	std::vector<std::vector<cv::Point> > hole_contour_saver;	//saving-vector for the found contours
