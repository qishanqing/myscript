...
void AbstractVoronoiSegmentation::mergeRooms(cv::Mat& map_to_merge_rooms, std::vector<Room>& rooms, double map_resolution_from_subscription, double max_area_for_merging, bool display_map,MapSegmentationGoal &goal,std::vector<std::vector<cv::Point>> &room_contours)
{
	...
	for(int i=0;i<room_contours.size();i++)
	{
		...
		{
			int current_id = map_to_merge_rooms.at<int>(room_contours[i][j].y, room_contours[i][j].x);
			if (current_id != 0)
			{
				for (size_t current_room = 0; current_room < rooms.size(); current_room++) //add the Points with the same Id as a room to it
				{
					...
					{
						rooms[current_room].insertMemberPoint(cv::Point(room_contours[i][j].x, room_contours[i][j].y), map_resolution_from_subscription);
						break;
					}
				}
			}
	...
	{
		rooms[current_room].setArea(cv::contourArea(room_contours[current_room])*map_resolution_from_subscription*map_resolution_from_subscription);
	...
	for (int current_room_index = 0; current_room_index < rooms.size(); )
	{
		...
		if (current_room.getNeighborCount() == 1 && current_room.getArea() < max_area_for_merging && current_room.getWallToPerimeterRatio() <= goal.wall_around_case_a)
		{
		...
		if (merge_rooms == true)//&& current_room.getArea()+rooms[merge_index].getArea()<20
		{
	...
	for (int current_room_index = 0; current_room_index < rooms.size(); )
	{
		...
		if (current_room.getArea() <goal.room_area_lower_limit && (double)current_room.getNeighborStatistics()[max_border_neighbor_id]/current_room.getPerimeter() > goal.contact_length_max_ratio_b)
		{
	...
	for (int current_room_index = 0; current_room_index < rooms.size(); )
	{
		...
		const int max_border_neighbor_id = current_room.getNeighborWithLargestCommonBorder();
		if ((current_room.getNeighborCount()==1 || current_room.getPerimeterRatioOfXLargestRooms(1)>goal.neighbor_room_length_max_ratio_c1) && current_room.getWallToPerimeterRatio() >goal.wall_around_case_c 
		&& (double)current_room.getNeighborStatistics()[max_border_neighbor_id]/current_room.getPerimeter() >goal.contact_length_max_ratio_c)
		{
			// merge with that neighbor that shares the most neighboring pixels
			merge_rooms = determineRoomIndexFromRoomID(rooms, max_border_neighbor_id, merge_index);
			if (rooms[merge_index].getNeighborCount() > 2 && rooms[merge_index].getPerimeterRatioOfXLargestRooms(2)<goal.neighbor_room_length_max_ratio_c2) // || rooms[merge_index].getWallToPerimeterRatio() < 0.4)
				merge_rooms = false;
		}
...
//		for (size_t n=0; n<neighbor_ids.size(); ++n)
		for (std::map< int,int,std::greater<int> >::iterator it=neighbor_room_statistics_inverse.begin(); it!=neighbor_room_statistics_inverse.end(); ++it)
		{
			...
			if (neighbor_border_ratio > goal.contact_length_max_ratio_d1 || (neighbor_border_ratio > goal.contact_length_max_ratio_d2 && current_room.getWallToPerimeterRatio() > (1-2*neighbor_border_ratio-0.05) && current_room.getWallToPerimeterRatio() < (1-neighbor_border_ratio)))
			{
				// merge with that neighbor that shares the most neighboring pixels
	...
	for (int current_room_index = 0; current_room_index < rooms.size(); )
	{
		...
		const int max_border_neighbor_id = current_room.getNeighborWithLargestCommonBorder();
		if ((double)current_room.getNeighborStatistics()[max_border_neighbor_id]/current_room.getPerimeter() > goal.contact_length_max_ratio_e)
		{
			// merge with that neighbor that shares the most neighboring pixels
			merge_rooms = determineRoomIndexFromRoomID(rooms, max_border_neighbor_id, merge_index);
		}
