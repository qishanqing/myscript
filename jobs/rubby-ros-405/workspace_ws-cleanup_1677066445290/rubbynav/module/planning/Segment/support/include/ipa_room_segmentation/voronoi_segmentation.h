...
public:
	...
	void segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map, double map_resolution_from_subscription,
			...
			double min_critical_point_distance_factor, double max_area_for_merging,std::vector < std::vector<cv::Point> > &saved_contours,MapSegmentationGoal &goal, bool display_map=false);
};
