...
void VoronoiSegmentation::segmentMap(const cv::Mat& map_to_be_labeled, cv::Mat& segmented_map, double map_resolution_from_subscription,
		...
		double min_critical_point_distance_factor, double max_area_for_merging,std::vector < std::vector<cv::Point> > &saved_contours,MapSegmentationGoal &goal, bool display_map)
{
...
//	cv::erode(voronoi_map_, voronoi_map_, cv::Mat(), cv::Point(-1, -1), 1);
	...
	//0.1s
		std::vector<std::vector<cv::Point>> room_contours;
	      //再次找区域
        #if CV_MAJOR_VERSION<=3
                    cv::findContours(segmented_map, room_contours, hierarchy, CV_RETR_CCOMP, CV_CHAIN_APPROX_NONE);
        #else
                cv::findContours(segmented_map, room_contours, hierarchy, cv::RETR_CCOMP, cv::CV_CHAIN_APPROX_NONE);
        #endif
		...
		mergeRooms(segmented_map, rooms, map_resolution_from_subscription, max_area_for_merging, display_map,goal,room_contours);
