...
//    return ::Empty(this->lidar);
    return this->orgImage.data.empty();
}
...
bool DataGroup::Remap()
{
    ...
    if (orgImage.data.empty()) return false;
    if (not DeDistortion::Remap(LEFT, image.data)) return false;
...
Lidar::Lidar(const swr::sensor_types::LaserScan data
...
bool Sector::Read(ifstream &stream)
{
...
bool Sector::Write(ofstream &stream) const
{
    if (not stream.is_open()) return false;
