...
bool GetData(const std::string inputDir, std::vector<SyncDataFile>& dataset)
{
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        if (synced) item = fileList.at(i).SetPrefix(inputDir + "/");
        else
        {
            item.image = inputDir + "/" + imageNameList.at(i);
...
bool ReadSyncFile(std::string srcFile, std::vector<SyncDataFile> &files)
{
    ...
    do
    {
        fin >> syncFile.image >> syncFile.imagePose >> syncFile.lidar
            >> syncFile.lidarPose;
