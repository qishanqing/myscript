...
bool Model::Init(const std::string modelFile, const DetectorOptions &options)
{
    ...
    this->option = options;
    inputSize.Set(options.param.input.width, options.param.input.height, options.param.input.channel);

    return CreateNetwork(modelFile, options.param.classNum);
...
bool Model::RunModel(void *input, void **output)
{
    Timer timer;
    bool flag = false;
