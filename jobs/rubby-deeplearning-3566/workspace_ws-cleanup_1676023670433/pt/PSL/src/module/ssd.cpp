...
void SSD::Tensor2Box(float *predictions)
{
#ifdef USE_NPU
    // todo
#elif USE_GPU
    ...
    auto anchor = predictions + outputSize.at(0);
    ...
    for (int i = 0; i < boxSize; ++i)
    {
        ...
        // location decoding
        float xcenter = boxes[iBox + classNum + 0] * mVariance[0] * anchor[iAnchor * 4 + 2] + anchor[iAnchor * 4 + 0];
        float ycenter = boxes[iBox + classNum + 1] * mVariance[1] * anchor[iAnchor * 4 + 3] + anchor[iAnchor * 4 + 1];
