...
Status LoadConfig(const std::string &configFile, ConfigParam &configParam)
{
    ...
    try
    {
        configParam.detect.type = DetectParam::DetectorType(
                configYaml["deeplearning"]["detect"]["modelType"].as<int>());
        configParam.detect.padding = configYaml["deeplearning"]["detect"]["padding"].as<bool>();
    }
    catch (const std::exception &e)
    {
        configParam.detect.type = DetectParam::UNKNOWN_DETECTOR;
        return Status(MISS_OPTION, "config option <detect.[modelType, padding]> missed");
    }
    ...
    catch (const std::exception &e)
    {
        return Status(MISS_OPTION
                      , "config option <structure/pose2Machine,lidarDirection> missed");
    }
