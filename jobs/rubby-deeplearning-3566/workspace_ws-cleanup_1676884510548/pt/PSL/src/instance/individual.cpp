...
// get roi lidar data by angle check
    CutSortEigen3DAngle(configParam, points, radianLeft + radianDelta,
            radianRight + radianDelta, pointsSelected
    );
}
...
psl::Location Individual::TriangleEstimate(BoxInfo &box, const int baseExtend)
{
    ...
    LOG_CHECK_DEBUG(INFO) << "befor y: " << box.box.y + box.height
                          << " last y: " << box.box.y + height;

    height = MIN(HEIGHT, box.box.y + height) - box.box.y;

    double ground = s * (box.box.y + height);
    const double z = 1.0;
    ...
    if (box.box.x < SOFT_BOUNDARY[0])
    {
        //move left
        for (int i = 0; i < 3; i++)
        {
            points(0, i) -= box.width * AMEND_THRESHOLD;
        }
    }
    if (box.box.y < SOFT_BOUNDARY[1])
    {
        //move bottom
        for (int i = 0; i < 3; i++)
        {
            points(1, i) += height * AMEND_THRESHOLD;
        }
    }

    if (box.box.x + box.box.width > 640 - SOFT_BOUNDARY[0])
    {
        //move right
        for (int i = 0; i < 3; i++)
        {
    ...
    float cx = K_T(0, 2);
    float cy = K_T(1, 2);
    if (centerGround(1) < cy) // ignore the point above the image half
    {
        return location;
    }
