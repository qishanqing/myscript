...
struct Location
{
    ...
    {
        char message[100];
        sprintf(message, "%.2f, %.2f, %.2f", x, y, z);
        return message;
    }
};
...
struct Object
{
    ...
    int id = -1;    // object ID
    ClassID classID = psl::DEFAULT_ID; // senmatic id
    float score;
    float detectExtend;
