...
struct StatusItem
{
    ...
    time_t lastDataTime = GET_TIMESTAMP;
    Message messageLack = Message(TEN_SECOND);
    Message messageReceive = Message(TEN_SECOND);
...
public:
    ...
    bool Init(const psl::Resolution resolution
...
private:
    ...
    */
    ...
    bool GetLidar(swr::psl_sensor_types::LaserScan &lidar);

    void Sync();

    void ThreadRun();

    void CleanHistory(const swr::psl_sensor_types::LaserScan &current);
...
private:
    ...
    std::list<swr::psl_sensor_types::LaserScan> lidars;
